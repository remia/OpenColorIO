...
#ifdef CHROMA_CURVE
    const float Mnorm = chromaCompressNorm(h, params.chroma_compress_scale);
#else
...
float compression_function(float x, float threshold, float limit, bool invert)
{
...
f3 compressGamut(const f3 &JMh, const OutputTransformParams& params, float Jx, bool invert)
{
    float limit_J_max = params.limit_J_max;
    float mid_J = params.mid_J;
    float focus_dist = params.focus_dist;

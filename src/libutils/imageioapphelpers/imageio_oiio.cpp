...
public:
    ...
    {
        read(filename, BIT_DEPTH_UNKNOWN);
    }
    ...
    void read(const std::string & filename, BitDepth bitdepth)
    {
        ...
        OIIO::TypeDesc typeDesc = OIIO::TypeDesc::UNKNOWN;

        // Use the specified bitdepth if requested.
        if (bitdepth != BIT_DEPTH_UNKNOWN)
        {
            typeDesc = BitDepthToTypeDesc(bitdepth);
        }
    ...
    void cropToFull()
    {
        OIIO::ImageBufAlgo::crop(m_buffer, m_buffer, get_roi_full(m_buffer.spec()));
    }

    void filterChannels(std::vector<int> chs)
    {
        OIIO::ImageBufAlgo::channels(m_buffer, m_buffer, chs.size(), chs.data());
    }

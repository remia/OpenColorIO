...
public:
    ...
    void write(const std::string & filename)
    {
        ...
        const std::vector<std::string> chanNames = getChannelNames();
        Imf::PixelType pixelType = BitDepthToPixelType(getBitDepth());
    ...
    void cropToFull()
    {
        throw Exception("Error: croptofull not supported with OpenEXR backend.");
    }

    void filterChannels(std::vector<int> /*chs*/)
    {
        throw Exception("Error: channel selection not supported with OpenEXR backend.");
    }
};
